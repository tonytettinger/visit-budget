...
(function () {
  ...
  if (btn) {
    var label = function () {
      btn.textContent = root.getAttribute('data-theme') === 'light' ? '○ light' : '● dark';
    };
    label();
    btn.addEventListener('click', function () {
      ...
      try { toLight ? localStorage.setItem(KEY, 'light') : localStorage.removeItem(KEY); } catch (e) {}
      label();
    });
  }
    ...
    bar.querySelectorAll('button[data-filter]').forEach(function (b) {
      b.addEventListener('click', function () {
        ...
        scope.querySelectorAll('[data-type]').forEach(function (item) {
          item.style.display = (on || f === 'all' || item.getAttribute('data-type') === f) ? '' : 'none';
        });
      });
    });
  });
